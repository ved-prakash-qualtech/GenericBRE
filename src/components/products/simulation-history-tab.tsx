  ...
  if (simulations.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 rounded-xl border border-dashed text-center">
        ...
        <p className="text-sm text-muted-foreground/70">Runs from the Run Simulator tab appear here, newest first</p>
      </div>
    );
  }
          ...
          return (
            <div key={sim.id} className="rounded-xl border bg-card transition-all duration-150 hover:border-primary/40 hover:shadow-2xs">
              ...
              >
                ...
                <OutcomeBadge outcome={sim.outcome} className="shrink-0 text-sm px-2 py-0.5" />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-foreground tracking-tight">{sim.summary}</p>
                  <p className="mt-0.5 text-sm text-muted-foreground">
                    ...
                    {sim.sandbox && <span className="ml-1 rounded bg-muted px-1.5 py-0.5 font-mono text-sm">sandbox</span>}
                  </p>
                </div>
                <span className="shrink-0 font-mono text-sm font-medium text-muted-foreground bg-muted/50 border border-border/80 px-2 py-0.5 rounded-md">
                  {sim.reasonCode}
                </span>
              </button>
              {expanded && (

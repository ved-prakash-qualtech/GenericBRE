...
export function RunSimulatorRedesigned({ product, sim, products = [], onProductChange }: RunSimulatorRedesignedProps) {
  ...
  const apiRequestJson = sim.jsonText || "{}";
  const apiResponseJson = result
    ? JSON.stringify(buildApiResponsePayload(result, sim.responseMode, sim.activeConfig), null, 2)
    : "";

  const lastUpdated = (() => {
    const raw = product.lastPublishedAt ?? product.updatedAt;
    if (!raw) return "—";
                  ...
                  <table className="w-full text-xs">
                    <tbody className="divide-y">
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-2.5 font-medium text-right">{lastUpdated}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    >
                      {copiedRequest ? <Check className="size-4 text-emerald-600" /> : <Copy className="size-4" />}
                    </Button>
                  </div>
                </div>
                <Textarea
              ...
              <div className="rounded-lg border bg-card p-3 space-y-2">
                ...
                <div className="space-y-0.5 text-[11px] border-b pb-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-emerald-600">{result ? passed : "—"}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="space-y-1.5">
                ...
                <div className="rounded-lg border bg-gray-900 p-2 font-mono text-[10px] text-gray-100 max-h-24 overflow-y-auto">
                  {result ? (
                    ...
                    <p className="text-gray-400">Run the simulation to see the decision response.</p>
                  )}
                </div>
              </div>
            </div>
